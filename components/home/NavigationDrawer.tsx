...
function NavDrawerItem({ isActive, name }: { isActive: boolean; name: string }) {
  return (
    <li
      className={cn(
        'flex h-[28px] w-full items-center justify-between rounded-[4px] border border-transparent bg-foreground pl-[14px] pr-2',
        isActive && 'border-white bg-white/40',
      )}
    >
      <div className={cn('size-3 rounded-full ', isActive && 'bg-maomaoyu-orange')} />
      <div className={cn('text-sm text-link-color', isActive && 'text-link-color')}>{name}</div>
    </li>
  );
}
...
export default function NavigationDrawer({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed right-0 top-16 z-[99999] h-[calc(100%-64px)] w-[276px] transform bg-maomaoyu-orange shadow-lg transition-transform duration-300',
